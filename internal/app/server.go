...
// Completion answers a completion request: function names in a formula,
// declared value names after `@`, nothing for a document that is not open.
func (s server) Completion(
	...
	text, ok := s.docs.get(params.TextDocument.URI)
	if !ok {
		return nil, nil //nolint:nilnil // "no completions here" is a nil result, not an error, per LSP.
	}
	s.logger.Debug("completion", slog.String("uri", string(params.TextDocument.URI)))
	return &protocol.CompletionList{Items: completions(text, params.Position)}, nil
}
...
// full-document change sync, hover, code actions, and completion.
	...
	return &protocol.InitializeResult{
		Capabilities: protocol.ServerCapabilities{
			...
			CompletionProvider: &protocol.CompletionOptions{
				// The sigil re-triggers completion so `@` flips the vocabulary
				// from functions to the sheet's declared names mid-word.
