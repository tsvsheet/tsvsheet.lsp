...
// TestDiagnoseNamedValueVerdicts pins the 023 seam in the editor: the
// engine's naming verdicts surface as diagnostics on their cells, so an
// editor cannot silently lose the feature's findings.
func TestDiagnoseNamedValueVerdicts(t *testing.T) {
	t.Parallel()

	diags := diagnose("=1 |@ named(X)\t=2 |@ named(x)\t=@Missing\n")
	messages := make([]protocol.InlayHintTooltip, 0, len(diags))
	for _, d := range diags {
		messages = append(messages, d.Message)
	}
	assert.Contains(t, messages, protocol.String("name x is already bound at A1"))
	assert.Contains(t, messages, protocol.String("unknown name: @Missing"))
	assert.Contains(t, messages, protocol.String("name X is bound but never used"))

	assert.Empty(t, diagnose("=0.08 |@ named(Rate)\t=@Rate * 100\n"),
		"the canonical named sheet is clean")
}
